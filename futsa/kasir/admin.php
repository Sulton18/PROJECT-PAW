<?php
	...
	$jml_data = mysql_num_rows(mysql_query("select * from kasir"));
	...
	if(isset($_GET['hal'])){
			...
			$s = mysql_query("select * from kasir order by id_user desc limit ".(($hal - 1) * $jml_per_halaman).", $jml_per_halaman");
		
	}else{
		$s = mysql_query("select * from kasir order by id_user desc limit 0, $jml_per_halaman")
				or die(mysql_error());
	}
?>
